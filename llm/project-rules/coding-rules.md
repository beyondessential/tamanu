...
- **Never put interaction event handlers on static elements.** For navigation, prefer `<a>` with an `href` attribute (or abstractions of it like `<Link>`). In other cases, use a semantic `<button>`.
