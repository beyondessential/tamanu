...
- **Never put interaction event handlers on static elements.** For navigation, prefer `<a>` with an `href` attribute (or abstractions of it like `<Link>`). In other cases, use a semantic `<button>`. e.g. A `<div>` or `<span>` should never have an `onClick` handler. If an has button semantics but absolutely needs to not look like one, make a `styled(UnstyledHtmlButton)`.
