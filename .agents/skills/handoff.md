...
2. Compose the briefing prompt and return it in a single fenced code block with ```handoff as the language tag. The UI renders these blocks with an **Open in Claude Code** button that launches a local Claude Code terminal session with this prompt pre-filled (via a `claude-cli://` deep link), alongside the standard copy button. **Critical: the entire prompt must be one fenced block. Do not use fenced code blocks (triple backticks) anywhere inside the prompt — they break the outer fence and disable both the button and copy. Use inline code with single backticks for paths, commands, and identifiers instead. Keep the prompt under 5000 characters so the deep link stays within the URL limit; the user can still copy-paste a longer briefing, but the button is disabled if the limit is exceeded.**
