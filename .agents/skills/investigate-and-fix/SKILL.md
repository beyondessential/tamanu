...
2. **Locate the code path** — use the specs in `specs/<area>/` to understand the intended behaviour, then find the code that implements (or should implement) it.
...
5. **Decide about the spec** — bug fixes usually don't need spec updates, but if the bug existed because the spec was unclear, wrong, or missing, update the spec first then fix the code to match (see `.agents/skills/shared/spec-format.md`).
