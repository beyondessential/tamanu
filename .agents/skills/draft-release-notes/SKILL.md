...
2. **🌟 Major Features and Changes** — the significant items, usually three to five, each with a real overview. Anything that gives users a new capability belongs here, even if that makes five. Judge scope by what shipped, not by how the card describes itself: a card calling its work "a couple of minor tweaks" often lands a full feature. Group by surface with `## ` headings, using only the surfaces that have content: **Tamanu Desktop**, **Tamanu Mobile**, **Patient Portal**, **System Administration**. Each feature is an `### _Italic title_` with an overview paragraph, a **Key features** bullet list, and a **Supporting documentation** list. Add **Security considerations** or a **Keen to implement …?** call-to-action block only where the feature warrants it (as the example does for the Patient Portal and integrations).
3. **🔧 System Enhancements** — improvements that make existing work better without adding a capability. Same shape as major features but lighter; group related items; call out any configuration change.
