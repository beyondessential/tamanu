...
- **Inverted commas around every feature and interface name**, spelled and capitalised as it appears on screen: the 'Send to pharmacy' column, the 'Dispense medication' modal, the 'Emergency patients' board, 'Last 7 days', 'Details' and 'Ordered by'. This covers buttons, fields, columns, tabs, modals, views, menu options, note types, named filter values, and feature names used as headings. It does **not** cover product surfaces (Tamanu Desktop, Tamanu Mobile, the admin panel, Patient Portal) or general concepts (an encounter, a price list, reference data), which read as ordinary nouns.
