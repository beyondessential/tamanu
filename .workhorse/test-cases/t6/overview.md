...
- [x] **The backfill survives two sensitive facilities sharing a code or a name.** Caught by the non-determinism job, which migrates seeded data holding two facilities called "Central Clinic": sensitive_networks holds code and name unique but facilities does not, so copying them verbatim broke the upgrade. The job itself is the regression guard — it runs the real migration over seeded data. verifies spec: SENSNET
