...
Divergences from a spec bullet rather than missing tests, so a test alone would not
close one. All five found in the audit are now closed: two in #10752, two in
#10754, and central orphan collection recorded as deferred in `reclamation.md`.

- [x] **A row the backfill has not reached serves the same way a moved one does.** Both servers' legacy paths go through `serveBlob`, so ranges, streaming and content-length match whichever form a row is in, and a ranged read no longer behaves differently per row mid-backfill. The extent comes from the bytes rather than the row's `size`, which the two disagree on. One difference stands with a reason: a row holding its own bytes has no content hash, so it carries no validator (verifies spec: BKFL, SERVE)
- [x] **A client that already holds a blob is told so rather than sent it again.** `serveBlob` answers 304 before opening the stream and sends `private, max-age=31536000, immutable`. Private deliberately: a shared cache holding clinical content would outlive the permission check that released it. The 304 repeats that freshness, since a cache updates its stored entry from the headers the 304 carries (verifies spec: SERVE)
