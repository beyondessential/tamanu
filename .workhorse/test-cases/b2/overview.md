...
- [ ] **Central orphan collection does not exist.** RECL's facility half (evict under an LRU size budget, never evict the outbox) is implemented and covered above. The central half has no implementation: no collector, no liveness scan across reference tables and changelog entries, no safety window, and no `spec: RECL` tag anywhere in the codebase. No card in the plan covered it. Deferral is defensible, since central accepts a blob only once a record references it, but nothing records the decision (verifies spec: RECL)
- [ ] **No operator surface for backfill progress.** BKFL says progress and completion are visible to operators as the amount moved and the amount remaining. The figures exist only in server logs: nothing in the query cookbook, the healthchecks map or any runbook. This is the one job that runs once, against real data, at up to hundreds of gigabytes (verifies spec: BKFL)
- [ ] **No runbook for a backfill rollback.** `addAttachmentHash.down` restores `NOT NULL` on `attachments.data`, so a downgrade fails while any hash-carrying row remains. The migration says so in a `DESTRUCTIVE:` comment, but no operator-facing document says to run `rollbackBlobBackfill` first (verifies spec: BKFL)
