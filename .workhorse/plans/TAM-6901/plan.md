...
Per-facility control of which item categories (imaging / lab / medications) are bundled into the admission fee and therefore **don't** auto-add for admission encounters — while still auto-adding for outpatient/ER. Discharge meds are always invoiced; procedures are never bundled; pre-admission items keep full price.

Implements the inpatient fee inclusions/exclusions section of `spec: FEES`. Needs the facility-settings block from the TAM-6898 foundation but is otherwise independent of the fee products, so it can run in parallel once that block lands.
...
- **Out of scope (confirmed by PM):** STAT medications administered to outpatients — whether they add an invoice line — are out of scope; no special handling is added for them.
