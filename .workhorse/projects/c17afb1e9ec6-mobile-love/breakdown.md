...
## Review IRD's facility and sync-scope setup · M5
...
## Run a performance sweep over the mobile app · N5
...
## Revisit program and form selector load time · P5

Forms take three to five minutes to appear on a lightly-populated program. An earlier fix worked for one user and the problem recurred for another on v0.5.29, so it did not reach the real cause. Profile the selector against IRD-shaped data on a low-spec device and fix it properly. Work has already started — see https://github.com/beyondessential/tamanu/pull/10825.
...
## Deliver app updates over the air · Q5

IRD find manual updates via the provided URL painful and asked for the Play Store. No in-app update mechanism exists today. Weigh Google Play Store, JS-bundle OTA (CodePush or expo-updates style), full APK delivery, and MDM-driven delivery against IRD's constraints — offline campsites, low-spec devices, intermittent connectivity, data residency — then implement the chosen mechanism: the update check, the prompt users see, forced versus optional update behaviour, and what happens on devices that have been offline for long stretches. Held as one card and broken down once the mechanism is chosen and the effort is sized. Carries the project's only design work, and may run past the cycle.

## Investigate unsynced forms lost when the mobile app updates · R5
...
## Investigate desktop forms missing after a sync · S5

Around fourteen or fifteen forms entered through Tamanu desktop did not appear after a sync and had to be re-entered. Distinct from the mobile case and more serious, since it undermines the digital-only record. Progress depends on IRD supplying dates and record details; follow up directly with whoever reported it if nothing lands within a couple of working days.

## Confirm mandatory questions only block submission when visible · T5
