...
- **Lab request view (encounter level).** Update the table so that it can handle displaying results for both individual tests and panels. Display individual tests first alphabetically followed by panels, alphabetically and within each panel display tests in the same order as listed in reference data
