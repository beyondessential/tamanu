...
**Desired behaviour.** List tests with a visibility status of 'panelOnly' in the lab test type manage tab on the admin panel. Also add 'Panel only' as an option to the visibilityStatus select field when managing a single lab test via the admin panel. 
