...
- **Lab request view (encounter level):**
  - Update the table so that it can handle displaying results for both individual tests and panels. Display individual tests first alphabetically followed by panels, alphabetically and within each panel display tests in the same order as listed in reference data. Please also update the tile 'Test category' to 'Category'. https://www.figma.com/design/sy6gyLBPoSXuJNq5lEEOL8/Tamanu-Desktop-1?node-id=41639-15551&t=tyhjuqgTUSzOAZGg-1
