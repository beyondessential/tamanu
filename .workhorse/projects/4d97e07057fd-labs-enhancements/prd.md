...
**Desired behaviour.** Support displaying a result such as `< 0.3` value wherever the result is shown, while still applying reference-range validation so the result flags as out of range.
