...
Behaviour has been drafted directly into `specs/blob-storage/attachments.md` (id ATCH), resolving the leans below as decisions. Two questions stay open: L2 release packaging, and the awaiting-content copy (deferred to spec review).
...
- [x] **Does `base64=true` survive?** Profile pictures and mobile's photo viewer rely on base64 JSON responses today. **Lean:** keep it — it is an attachment-endpoint compatibility concern, not a blob-layer one. Endpoint streams with range support by default per SERVE; base64 retained where callers need it. Migrating those callers is out of scope.
- [x] **Upload guards.** The disk half is covered: CAP specifies admission refusal at the free-disk reserve, replacing `canUploadAttachment`. **Lean:** keep `maxFileSize` as the attachment-domain product guard, enforced at admission on the facility. Only the user-facing rejection copy is J2's to spec.
...
- **Scope-tightening has no victim paths — verified.** Concern from spec review: BLAC means a facility can only fetch blobs whose referencing records are in its sync scope, tighter than today's proxy-anything behaviour. Walked every read path (document metadata, lab attachments, patient letters, survey photos, profile pictures): each starts from a locally-synced owning record, so the reference is in scope by construction. Profile pictures specifically are a survey answer (`ProfilePhoto`-coded data element, queried from local survey_response_answers) feeding the printed patient ID card — only populated on deployments configured with that survey question, which is why dev environments only ever show letter avatars.
