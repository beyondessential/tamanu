...
- [ ] History is reconstructed from the answer's changelog entries. Consecutive entries supply each edit's before and after values, and the answer's insert entry supplies the original recording.
  - [ ] The original recording carries the observation time and the recording clinician, read from the response. An insert entry authored while applying a sync carries no audit user and the time of that sync, so it is not the source for that line.
