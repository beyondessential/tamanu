...
Until a facility has obtained the key it can still authenticate and synchronise — obtaining sync access does not require the settings PSK — but it cannot decrypt secret settings, and it keeps retrying until it obtains the key. The central server discloses the settings PSK only to a caller authenticated as a facility authorised to synchronise; it is never returned to an unauthenticated caller.
