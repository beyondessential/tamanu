...
A facility obtains the settings PSK from the central server over an authenticated channel, as part of establishing its sync credentials. A freshly installed facility receives it during setup; a facility that was configured before the shared key existed receives it on its next synchronisation, so a facility whose central server was unreachable during an upgrade converges as soon as it is reachable again rather than waiting for another upgrade.

Until a facility has obtained the key it can still authenticate and synchronise — obtaining sync access does not require the settings PSK — but it cannot decrypt secret settings, and it keeps retrying until it obtains the key.

The central server discloses the settings PSK only to the dedicated sync user a facility server authenticates as, and never to a person. Full administrative rights are required and are not enough on their own: a human administrator holding the same rights is refused, on any client. This matters because facility servers and mobile devices are granted the same sync access, so being allowed to synchronise does not establish that a caller is a facility server; the distinguishing property is that the central server itself created the account when it issued that facility's sync credentials. A facility still authenticating with credentials carried over from a configuration file exchanges them for a dedicated sync user before it can obtain the key.
