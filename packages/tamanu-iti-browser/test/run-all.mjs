...
import { launchChrome, pageTargetWs, CDP, sleep } from './cdp.mjs';
...
try {
  ...
  const health = await (await fetch(`http://127.0.0.1:${goodConn.proxyPort}/health`)).text();
  ...
  const wsEcho = await new Promise(resolve => {
    const ws = new WebSocket(`ws://127.0.0.1:${goodConn.proxyPort}/ws`);
  ...
  // ---- Browser: secure context, storage, *.localhost, in-page WS ---------
