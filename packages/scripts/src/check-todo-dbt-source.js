...
async function getSchemaPaths(packageName) {
  const packagePath = path.join('database/model', packageName);
  return (await fs.readdir(packagePath)).map(schemaName => path.join(packagePath, schemaName));
}

async function detectTodoItemsInTable(schemaPath, dbtSrc) {
  const table = dbtSrc.sources[0].tables[0];
  const isDescriptionTodo = dbtSrc.sources[0].description === 'TODO';
  if (isDescriptionTodo) {
    console.log(`TODO: table description for ${table.name}, in ${schemaPath}/${table.name}.yml'`);
  }

  const doc = await readTableDoc(schemaPath, table.name);
  if (doc === null) return;

  const isDocTodo = doc.description === 'TODO';
  if (isDocTodo) {
    console.log(`TODO: table documentation for ${table.name}, in ${schemaPath}/${table.name}.md`);
  }

  const todoColumnDocs = doc.columns.filter(c => c.description === 'TODO');
  for (const column of todoColumnDocs) {
    console.log(
      `TODO: column documentation for ${table.name}:${column.name}, in ${schemaPath}/${table.name}.md`,
    );
  }
...
async function run(packageName) {
  const promises = (await getSchemaPaths(packageName)).map(async schemaPath => {
    const detectPromises = (await readTablesFromDbt(schemaPath)).map(s =>
      detectTodoItemsInTable(schemaPath, s),
    );
