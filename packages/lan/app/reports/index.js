...
const diagnosesByVillageReport = async (db, { startDate, endDate, village }) => {
  const baseDiagnoses = db.objects('patientDiagnosis')
    .filtered("date >= $0 AND date <= $1 AND visit.patient.village.name = $2", startDate, endDate, village)

  const counts = {};

  const rowData = baseDiagnoses.forEach(d => {
