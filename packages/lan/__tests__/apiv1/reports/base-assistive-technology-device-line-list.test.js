...
describe('Assistive technology device line list', () => {
  ...
  describe('returns the correct data', () => {
    it('should return latest data per patient and latest data per patient per date', async () => {
      ...
      //-----Row on 2021-03-17-----//, on this date, there are answers submitted for 2 patients
      //**PATIENT 1**
      //patient details
      ...
      //**PATIENT 2**
      //patient details
      expect(result.body[2][0]).toBe(expectedPatient2.displayId);
      expect(result.body[2][1]).toBe(expectedPatient2.sex);
      expect(result.body[2][2]).toBe(moment(expectedPatient2.dateOfBirth).format('DD-MM-YYYY'));

      //always grab the latest answer for a data element for that patient.
      expect(result.body[2][4]).toBe('pde-IrqMAReg-13-on-2021-03-12T10:53:15.708Z-Patient2');

      //always grab the latest answer for a data element within that date, regardless of survey response,
      expect(result.body[2][5]).toBe('pde-IrqPreMob-2-on-2021-03-17T10:53:15.708Z-Patient2');
      expect(result.body[2][6]).toBe('pde-IrqPreMob-1-on-2021-03-17T10:53:15.708Z-Patient2');
      expect(result.body[2][7]).toBe('pde-IrqPreMob-6-on-2021-03-17T10:53:15.708Z-Patient2');
      expect(result.body[2][8]).toBe(null);
      expect(result.body[2][9]).toBe(null);
      expect(result.body[2][10]).toBe(null);

      //-----Row on 2021-03-20-----//, on this date, there are answers submitted for only 1 patient
      //**PATIENT 1**
      //patient details
      expect(result.body[3][0]).toBe(expectedPatient1.displayId);
      expect(result.body[3][1]).toBe(expectedPatient1.sex);
      expect(result.body[3][2]).toBe(moment(expectedPatient1.dateOfBirth).format('DD-MM-YYYY'));

      //always grab the latest answer for a data element for that patient.
      expect(result.body[3][4]).toBe('pde-IrqMAReg-13-on-2021-03-15T10:53:15.708Z-Patient1');

      //always grab the latest answer for a data element within that date, regardless of survey response
      expect(result.body[3][5]).toBe('pde-IrqPreMob-2-on-2021-03-20T10:53:15.708Z-Patient1');
      expect(result.body[3][6]).toBe('pde-IrqPreMob-1-on-2021-03-20T10:53:15.708Z-Patient1');
      expect(result.body[3][7]).toBe('pde-IrqPreMob-6-on-2021-03-20T10:53:15.708Z-Patient1');
      expect(result.body[3][8]).toBe('pde-IrqPreMob-7-on-2021-03-20T11:53:15.708Z-Patient1');
      expect(result.body[3][9]).toBe('pde-IrqPreMob-8-on-2021-03-20T11:53:15.708Z-Patient1');
      expect(result.body[3][10]).toBe('pde-IrqPreMob-9-on-2021-03-20T11:53:15.708Z-Patient1');
    });
