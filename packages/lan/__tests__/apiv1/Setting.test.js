...
describe.only('Setting', () => {
  ...
  describe('GET setting/:key', () => {
    ...
    it('fetches a setting record within a facility', async () => {
      const facility = await models.Facility.create(fake(models.Facility));
      await Setting.set(TEST_KEY, TEST_VALUE, facility.id);

      const result = await app.get(`/v1/setting/${TEST_KEY}?facilityId=${facility.id}`).send({});

      expect(result).toHaveSucceeded();
      expect(result.body.data).toEqual(TEST_VALUE);
    });

    it('does not fetch a setting record within a different facility', async () => {
      ...
      const result = await app.get(`/v1/setting/${TEST_KEY}?facilityId=${facility2.id}`).send({});
