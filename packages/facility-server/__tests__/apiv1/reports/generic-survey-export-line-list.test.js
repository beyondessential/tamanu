...
describe('Generic survey export line list report', () => {
  ...
  describe('Soft-deleted survey responses', () => {
    ...
    it('includes soft-deleted survey responses when includeDeletedResponses is true', async () => {
      const {
        Facility,
        Location,
        Department,
      ...
      await createResponseForPatient();
      const deletedResponse = await createResponseForPatient();
      await deletedResponse.destroy();
      ...
      app = await baseApp.asNewRole(permissions);
      const result = await app.post(`/api/reports/${reportVersionId}`).send({
        parameters: { surveyId: deletionSurvey.id, includeDeletedResponses: true },
      });
