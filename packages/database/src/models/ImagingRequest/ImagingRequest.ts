...
export class ImagingRequest extends Model {
  ...
  /** Present when selected via {@link getApprovedAttribute}; not a DB column. */
  ...
  /**
   * Computed invoice-approval status for use in `attributes.include`.
   * ImagingRequestArea items take precedence, then ImagingRequest items.
   * @returns {[import('sequelize').Literal, 'approved']}
   */
  static getApprovedAttribute() {
    return [
      literal(`(
        SELECT COALESCE(
  ...
  /**
   * Build a Sequelize order clause, with special handling for computed literal fields.
   */
  static getOrder(
    orderBy?: string,
    order: string = 'ASC',
    options: { literalSortKeys?: string[] } = {},
  ) {
