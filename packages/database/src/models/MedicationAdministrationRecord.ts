...
import { ADMINISTRATION_FREQUENCIES, ENCOUNTER_TYPES, SYNC_DIRECTIONS, SYSTEM_USER_UUID, TASK_STATUSES, TASK_TYPES, MEDICATION_DUE_TASK_NAME } from '@tamanu/constants';
import {
  addDays,
  addHours,
  addMonths,
...
export class MedicationAdministrationRecord extends Model {
  ...
  static async generateMedicationAdministrationRecords(prescription: Prescription) {
    ...
    if (prescription.frequency === ADMINISTRATION_FREQUENCIES.IMMEDIATELY) {
      if (!lastMedicationAdministrationRecord) {
        ...
        await this.createMedicationDueTaskForMar(prescription, prescription.startDate);
      }
      return;
    }
    ...
    while (lastDueDate < endDate) {
      for (const idealTime of prescription.idealTimes || []) {
        ...
        if (isValid(nextDueDate)) {
          ...
          await this.createMedicationDueTaskForMar(prescription, nextDueDate);
        }
      }
  ...
  /**
   * Creates a medication due task
   * @param prescription
   * @param dueAt
   */
  static async createMedicationDueTaskForMar(prescription: Prescription, dueAt: Date | string) {
    const { models } = this.sequelize;
    const { Task, Encounter, EncounterPrescription } = models;

    // Skip if this is a PRN medication
    if (prescription.isPrn) return;

    const dueTime = toDateTimeString(new Date(dueAt));
    if (!dueTime) return;
    ...
    if (!encounterPrescription?.encounter) return;
    
    const encounter = encounterPrescription.encounter;
    
    // Skip if this is not an inpatient encounter (admission type and not discharged)
    if (encounter.encounterType !== ENCOUNTER_TYPES.ADMISSION || encounter.endDate) return;
    ...
    // Skip if the task at the same time already exists
