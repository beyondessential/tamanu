...
import { PERMISSION_SCHEMA, VERB_ABBREVIATIONS } from '@tamanu/constants';
...
const VERB_HIERARCHY = ['delete', 'create', 'write', 'list', 'read'];
...
export const NounSection = ({ nounGroup, selectedRoles, onToggle, objectNames }) => {
  ...
  const isChecked = (verb, roleId) => {
    const key = `${verb}#${roleId}`;
    if (key in localOverrides) return localOverrides[key];
    return !!nounGroup.verbs.find(v => v.verb === verb)?.roles[roleId];
  };
  ...
  const handleToggle = (verb, roles, role) => {
    const currentValue = isChecked(verb, role.id);
    const newValue = !currentValue;
    const overrides = { [`${verb}#${role.id}`]: newValue };
    const toggles = [
      {
        verb,
        noun: nounGroup.noun,
    ...
    if (newValue) {
      for (const implied of getImpliedVerbs(verb)) {
        if (availableVerbs.has(implied) && !isChecked(implied, role.id)) {
          overrides[`${implied}#${role.id}`] = true;
          toggles.push({
            verb: implied,
            noun: nounGroup.noun,
            objectId: nounGroup.objectId,
    ...
    <>
      <NounRow onClick={() => setExpanded(prev => !prev)}>
        ...
        {selectedRoles.map(role => (
          <SummaryCell key={role.id}>{getLocalSummary(role.id)}</SummaryCell>
        ))}
      </NounRow>
      {expanded &&
        nounGroup.verbs.map(({ verb, roles }) => (
          <VerbRow key={verb}>
            <EmptyChevronCell />
