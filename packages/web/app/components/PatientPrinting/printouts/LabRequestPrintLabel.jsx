...
export const LAB_LABEL_PREVIEW_DIMENSIONS = { width: 320, height: 224 };
const { width: PREVIEW_WIDTH_PX, height: PREVIEW_HEIGHT_PX } = LAB_LABEL_PREVIEW_DIMENSIONS;

const LABEL_PADDING_RATIO = 0.05;

const TEXT_VIEWBOX_WIDTH = PREVIEW_WIDTH_PX * (1 - 2 * LABEL_PADDING_RATIO);
...
const FlexContainer = styled.div`
  ...
  padding: ${LABEL_PADDING_RATIO * 100}%;
  display: flex;
...
const TextContainer = styled.div`
  svg {
    // Fills the label content width in both variants, so the print is an exact scaled-down preview.
    width: 100%;
  }
...
export const LabRequestPrintLabel = React.memo(({ data, variant = 'print' }) => {
  ...
  const dateOfBirth = patientDateOfBirth
    ...
    : '';
  return (
    <Container $variant={variant} data-testid="container-gx0i">
...
LabRequestPrintLabel.propTypes = {
  ...
  variant: PropTypes.oneOf(['print', 'preview']),
};
