...
export const ButtonRow = React.memo(({ children, ...props }) => (
  <Row
    items={Children.toArray(children).length || 1}
    {...props}
    data-testid='row-atzb'>
    {children}
  </Row>
));
...
export const FormSubmitCancelRow = React.memo(
  ({
    ...
    <ButtonRow {...props} data-testid='buttonrow-4wf7'>
      {onCancel && <FormCancelButton onClick={onCancel} data-testid='formcancelbutton-aseq'>{cancelText}</FormCancelButton>}
      {CustomConfirmButton ? (
        <CustomConfirmButton
          onClick={onConfirm}
...
export const ConfirmCancelRow = React.memo(
  ({
    ...
    <ButtonRow {...props} data-testid='buttonrow-lft2'>
      {onCancel && <OutlinedButton onClick={onCancel} data-testid='outlinedbutton-95wy'>{cancelText}</OutlinedButton>}
      <ConfirmButton
        color={confirmColor}
        onClick={onConfirm}
        disabled={confirmDisabled}
  ...
  <FlexSpaceBetween data-testid='flexspacebetween-p9lh'>
    {onBack && (
      <GoBackButtonContainer data-testid='gobackbuttoncontainer-pfqw'>
        <OutlinedButton onClick={onBack} data-testid='outlinedbutton-fa5a'>{backButtonText}</OutlinedButton>
    ...
    {onBack && (
      <GoBackButtonContainer data-testid='gobackbuttoncontainer-79x5'>
        <OutlinedButton
          ...
          data-testid='outlinedbutton-1xr6'>
          {backButtonText}
        </OutlinedButton>
      </GoBackButtonContainer>
    )}
