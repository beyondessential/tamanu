...
export const PharmacyOrderModal = React.memo(
  ({ encounter, patient, ongoingPrescriptions, open, onClose, onSubmit }) => {
        ...
        return (ongoingPrescriptions || [])
          ...
          .map(prescription => {
            ...
            return {
              ...
              isSelectionDisabled:
                !canEditRepeats && (prescription.repeats ?? 0) === 0 && lastOrderedAt,
            };
          });
      }
    ...
    const handleSendOrder = useCallback(
      async (skipAlreadyOrderedCheck = false) => {
        if (!validateForm()) return;

        if (!skipAlreadyOrderedCheck && getAlreadyOrderedPrescriptions().length > 0) {
