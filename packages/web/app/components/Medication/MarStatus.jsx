...
export const MarStatus = ({
  ...
  const { dueAt, recordedAt, status, reasonNotGiven, doses, isAutoGenerated } = marInfo || {};
  ...
  const isRecordedOutsideAdministrationSchedule =
    !isAutoGenerated || new Date(recordedAt) < new Date(dueAt);
  const isDoseAmountNotMatch =
    !isVariableDose &&
    status === ADMINISTRATION_STATUS.GIVEN &&
    marInfo?.doses?.some(dose => Number(dose.doseAmount) !== Number(doseAmount));
