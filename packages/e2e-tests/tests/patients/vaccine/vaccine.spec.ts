...
test.describe('Vaccines', () => {
  test.describe('Without active admission', () => {
    ...
    test('[T-0441][AT-1017]Edit unique fields for other vaccine', async ({
      patientDetailsPage,
    }) => {
  ...
  test.describe('With active admission', () => {
    test.describe.configure({ mode: 'parallel' });
    ...
    test('[T-0435][AT-1006]Record given elsewhere for Routine vaccine', async ({
      ...
      await testGivenElsewhereForCategory(patientDetailsPage, 'Routine');
    });

    test('[T-0435][AT-1007]Record given elsewhere for Catchup vaccine', async ({
      patientDetailsPage,
    }) => {
      await testGivenElsewhereForCategory(patientDetailsPage, 'Catchup');
    });

    test('[T-0435][AT-1008]Record given elsewhere for Campaign vaccine', async ({
      patientDetailsPage,
    }) => {
      await testGivenElsewhereForCategory(patientDetailsPage, 'Campaign');
    });

    test('[T-0437][AT-1009]Record given elsewhere for Other vaccine', async ({
      patientDetailsPage,
    }) => {
      await testGivenElsewhereForCategory(patientDetailsPage, 'Other');
    });
