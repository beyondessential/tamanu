...
async function createAndNavigateToPatient(allPatientsPage: any, api: any) {
  ...
  await allPatientsPage.recentlyViewedPatientsList.waitForFirstRecentlyViewedPatientToHaveNHN(newPatient.displayId);
  return newPatient;
}
...
async function verifyRecentlyViewedPatients(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (i == 0 && expectedPatient) {
      await allPatientsPage.recentlyViewedPatientsList.waitForFirstRecentlyViewedPatientToHaveNHN(expectedPatient.nhn);
    }

    const recentlyViewedPatient = await allPatientsPage.recentlyViewedPatientsList.getRecentlyViewedPatientByIndex(i);
    ...
    if (expectedPatient) {
      ...
      const expectedFormattedDate = allPatientsPage.recentlyViewedPatientsList.formatDateForRecentlyViewed(expectedPatient.dateOfBirth || expectedPatient.formattedDOB || '');
...
test.describe('Recently viewed patients details', () => {
  ...
  test('[AT-0133]Most recently viewed patient appears at the top of the list and confirms the correct patient data in the list', async ({ allPatientsPage, api }) => {
    ...
    await expect(allPatientsPage.recentlyViewedPatientsList.firstRecentlyViewedGender).toHaveText(new RegExp(`^${newPatient.sex}$`, 'i'));
    const expectedFormattedDate = allPatientsPage.recentlyViewedPatientsList.formatDateForRecentlyViewed(newPatient.dateOfBirth || '');
  ...
  test('[AT-0134]Most recently viewed patient appears at the top of the list', async ({ allPatientsPage, api }) => {
    ...
    await expect(allPatientsPage.recentlyViewedPatientsList.firstRecentlyViewedName).toHaveText(secondPatient.firstName + ' ' + secondPatient.lastName);
...
test.describe('Recently viewed patient colors', () => {
  ...
  test('[AT-0138]The patient with clinic admission color is yellow in recently viewed list', async ({allPatientsPage, api }) => {
    ...
    await allPatientsPage.patientTable.clickOnFirstRow();
    await allPatientsPage.goto();
    await allPatientsPage.recentlyViewedPatientsList.waitForFirstRecentlyViewedPatientToHaveColor(PATIENT_COLORS.CLINIC_ADMISSION, 20000);
    const color = await allPatientsPage.recentlyViewedPatientsList.getRecentlyViewedPatientNameColor();
  ...
  test('[AT-0139]The patient with hospital admission color is green in recently viewed list', async ({allPatientsPage, api }) => {
    ...
    await allPatientsPage.patientTable.clickOnFirstRow();
    await allPatientsPage.goto();
    await allPatientsPage.recentlyViewedPatientsList.waitForFirstRecentlyViewedPatientToHaveColor(PATIENT_COLORS.HOSPITAL_ADMISSION, 20000);
    const color = await allPatientsPage.recentlyViewedPatientsList.getRecentlyViewedPatientNameColor();
  ...
  test('[AT-0140]The patient with triage admission color is red in recently viewed list', async ({allPatientsPage, api }) => {
    ...
    await allPatientsPage.patientTable.clickOnFirstRow();
    await allPatientsPage.goto();
    await allPatientsPage.recentlyViewedPatientsList.waitForFirstRecentlyViewedPatientToHaveColor(PATIENT_COLORS.TRIAGE_ADMISSION, 20000);
    const color = await allPatientsPage.recentlyViewedPatientsList.getRecentlyViewedPatientNameColor(); 
  ...
  test('[AT-0141]The patient with no admission color is blue in recently viewed list', async ({allPatientsPage, api }) => {
    ...
    await allPatientsPage.patientTable.clickOnFirstRow();
    await allPatientsPage.goto();
    await allPatientsPage.recentlyViewedPatientsList.waitForFirstRecentlyViewedPatientToHaveColor(PATIENT_COLORS.NO_ADMISSION, 20000);
    const color = await allPatientsPage.recentlyViewedPatientsList.getRecentlyViewedPatientNameColor();   
