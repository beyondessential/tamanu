...
test.describe('Notes Tests', () => {
  ...
  test.describe('Edit Note Tests', () => {
    test('[T-0191][AT-0081]should edit an existing note and view the change log', async ({
      ...
      // Dates: header uses `revisedBy.date` (newest). Changelog rows are newest-first; first() is the latest revision.
      // That time can match nth(1) when both edits fall in the same displayed minute.
