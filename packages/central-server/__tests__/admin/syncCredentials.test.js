...
describe('Admin sync credentials', () => {
  ...
  it('rejects unauthenticated requests', async () => {
    const result = await baseApp.post(ENDPOINT).send({ facilityIds: ['facility-a'] });
  ...
  it('rotates the same account (new password) on repeat calls for the same facilities', async () => {
    const app = await baseApp.asRole('admin');
    const facilityIds = ['facility-cred-rotate'];

    const first = await app.post(ENDPOINT).send({ facilityIds });
    const second = await app.post(ENDPOINT).send({ facilityIds });
