...
describe('Self-referencing foreign keys on syncable tables must be DEFERRABLE', () => {
  ...
  it('all self-referencing FKs on syncable tables are deferrable', async () => {
    ...
    const nonDeferrable = selfRefFks.filter(fk => !fk.is_deferrable);

    if (nonDeferrable.length > 0) {
      const details = nonDeferrable
        .map(fk => `  ${fk.table_name}.${fk.column_name} (${fk.constraint_name})`)
        .join('\n');
      throw new Error(
        `Self-referencing foreign keys on syncable tables must be DEFERRABLE.\n` +
