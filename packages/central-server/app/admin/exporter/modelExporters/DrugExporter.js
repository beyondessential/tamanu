import { FACILITY_DRUG_QUANTITY_STATUS, REFERENCE_TYPES } from '@tamanu/constants';
...
export class DrugExporter extends ReferenceDataExporter {
  async getData() {
    ...
    const allFacilityIds = [... new Set(drugs.flatMap(drug => drug.referenceDrug?.facilities?.map(facility => facility.facilityId) ?? []))];
    return drugs.map((drug) => {
      ...
      for (const facilityId of allFacilityIds) {
        baseData[facilityId] = FACILITY_DRUG_QUANTITY_STATUS.UNKNOWN;
      }
      
      if (drug.referenceDrug?.facilities) {
        drug.referenceDrug.facilities.forEach((facility) => {
          baseData[facility.facilityId] = facility.quantity ?? FACILITY_DRUG_QUANTITY_STATUS.UNKNOWN;
        });
      }
