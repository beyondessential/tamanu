...
export const snapshotGlobalChangesForModel = async (model, config, since, sessionConfig) => {
  ...
  const { patientIdTable, globalFilter: filter } = model.buildPatientSyncSelectQuery(sessionConfig);
...
export const triggerGlobalSnapshot = withConfig(async (outgoingModels, since, config) => {
