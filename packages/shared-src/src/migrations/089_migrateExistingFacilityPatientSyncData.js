...
module.exports = {
  up: async query => {
    ...
    await query.sequelize.query(`
      INSERT INTO patient_facilities (id, patient_id, facility_id)
      SELECT (${uuidgen}), patients.id, '${config.serverFacilityId}'
      FROM patients
      WHERE patients.marked_for_sync = TRUE;
    `);
    await query.sequelize.query(`
      ALTER TABLE patients DROP COLUMN marked_for_sync;
    `);
  },
  down: async query => {
