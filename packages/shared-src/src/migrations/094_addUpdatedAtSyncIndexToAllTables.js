...
module.exports = {
  ...
  down: async query => {
    ...
    for (const table of syncingTables) {
      await query.sequelize.query(`
        DROP TRIGGER IF EXISTS set_${table}_updated_at_sync_index_on_insert ON ${table};
      `);
      await query.sequelize.query(`
        DROP TRIGGER IF EXISTS set_${table}_updated_at_sync_index_on_update ON ${table};
      `);
