...
module.exports = {
  up: async query => {
    ...
    for (const table of syncingTables) {
      // add the updated_since_session column
      await query.addColumn(table, 'updated_since_session', {
        type: INTEGER,
      });
      ...
      await query.sequelize.query(`
        ALTER TABLE ${table} ALTER COLUMN updated_since_session SET NOT NULL;
      `);

      // before insert, set updated_at if not provided
      await query.sequelize.query(`
        CREATE TRIGGER set_${table}_updated_since_session_on_insert
      ...
      await query.sequelize.query(`
        CREATE TRIGGER set_${table}_updated_since_session_on_update
  ...
  down: async query => {
    ...
    for (const table of syncingTables) {
      await query.sequelize.query(`
        DROP TRIGGER set_${table}_updated_since_session_on_insert ON ${table};
      `);
      await query.sequelize.query(`
        DROP TRIGGER set_${table}_updated_since_session_on_update ON ${table};
      `);
      await query.removeColumn(table, 'updated_since_session');
    }
  },
};
