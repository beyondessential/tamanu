...
export async function down(query) {
  for (const upstream of UPSTREAMS) {
    query.sequelize.query(`DROP TRIGGER IF EXISTS fhir_job_queue_${upstream} ON ${upstream}`);
  }

  for (const upstream of UPSTREAMS) {
    query.sequelize.query(`DROP FUNCTION IF EXISTS fhir_queue_trigger_${upstream}()}`);
  }
}
