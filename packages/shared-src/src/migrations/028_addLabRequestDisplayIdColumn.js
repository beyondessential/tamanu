...
module.exports = {
  up: async query => {
    ...
    // this monster of a query takes the first six characters of a UUID
    // (guaranteed to match /[0-9a-f]{6}/), uppercases it, and replaces the
...
UPDATE lab_requests
  SET display_id = replace(replace(substring(upper(id) FROM 1 for 6), '0', 'G'), '1', 'H')
