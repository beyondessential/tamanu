...
async function updateTableColumn(query: QueryInterface, table: string, column: string) {
...
async function downdateTableColumn(query: QueryInterface, table: string, column: string) {
  await query.sequelize.query(`
    ALTER TABLE ${table}
