...
export const getAllergyIntolerances = async ({ patient, models, dataDictionariesIps }) => {
    ...
    return [
      {
        ...
        text: {
          status: 'generated',
          div: `<div xmlns="http://www.w3.org/1999/xhtml">These are the Allergy Intolerance details for ${escapeHtml(patient.displayName)} for ${escapeHtml(allergyCodingDisplay)}. Please review the data for more detail.</div>`,
        },
      },
    ];
  }
  ...
  return patientAllergies.map(patientAllergy => ({
    ...
    text: {
      status: 'generated',
      div: `<div xmlns="http://www.w3.org/1999/xhtml">These are the Allergy Intolerance details for ${escapeHtml(patient.displayName)} for ${escapeHtml(patientAllergy.allergy.name)}. Please review the data for more detail.</div>`,
    },
  }));
};
