...
describe('build Ability from permission definition', () => {
  ...
  it('should build an Ability that understands reports', async () => {
    ...
    expect(ability.cannot('unrecognised-verb', { type: 'Report', id: 'report-allowed' })).toEqual(
      true,
    );
  });
  ...
  it('should build an Ability that understands surveys', async () => {
    ...
    expect(ability.cannot('submit', { type: 'UnrecognisedType', id: 'survey-allowed' })).toEqual(
      true,
    );
    expect(ability.cannot('view', { type: 'UnrecognisedType', id: 'survey-allowed' })).toEqual(
      true,
    );
    expect(ability.cannot('unrecognised-verb', { type: 'Survey', id: 'survey-allowed' })).toEqual(
      true,
    );
  });
});
