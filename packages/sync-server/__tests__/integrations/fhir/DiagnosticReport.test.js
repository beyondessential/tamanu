...
// This is basically a copy of the Fiji VPS test, except it's
// pointed at the FHIR integration and doesn't set the client headers.
// it's meant to verify we keep the same functionality.
describe('FHIR integration - Immunization', () => {
  ...
  describe('success', () => {
    it('fetches a diagnostic report', async () => {
      ...
      const path = `/v1/integration/fhir/DiagnosticReport?_sort=-issued&_page=0&_count=2&status=final&subject%3Aidentifier=${id}&_include=DiagnosticReport%3Aresult&_include=DiagnosticReport%3Aresult.device%3ADevice`;
    ...
    it('fetches multiple pages', async () => {
      ...
      const id = encodeURIComponent(`${IDENTIFIER_NAMESPACE}|${patient.displayId}`);
      const path = `/v1/integration/fhir/DiagnosticReport?_sort=-issued&_page=0&_count=2&status=final&subject%3Aidentifier=${id}`;

      // act
      const response1 = await app.get(path);

      const nextUrl = response1.body.link.find(l => l.relation === 'next')?.url;
      const [, nextPath] = nextUrl.match(/^.*(\/v1\/integration\/fhir\/.*)$/);
      ...
      expect(response1.body).toMatchObject({
        ...
        link: [
          ...
          {
            relation: 'next',
            url: expect.stringContaining('/v1/integration/fhir/DiagnosticReport?searchId='),
          },
        ],
        entry: [
          {
  ...
  describe('failure', () => {
    it('returns a 422 error when passed the wrong query params', async () => {
      ...
      const path = `/v1/integration/fhir/DiagnosticReport?_sort=deceased&_page=-1&_count=101&status=invalid-status&subject%3Aidentifier=${id}&_include=DiagnosticReport%3Asomething-invalid`;
    ...
    it('returns a 422 error when passed no query params', async () => {
      ...
      const path = `/v1/integration/fhir/DiagnosticReport`;
