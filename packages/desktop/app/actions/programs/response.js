...
import { LOAD_RESPONSE_REQUEST, LOAD_RESPONSE_SUCCESS, LOAD_RESPONSE_FAILED } from '../types';
import { ProgramModel, PatientModel, SurveyResponseModel } from '../../models';

export const initResponse = ({ patientId, programId, surveyId, responseId }) => async dispatch => {
  ...
  const [error] = await to(Promise.all([patient.fetch(), program.fetch(), response.fetch()]));
