...
export class AuthService {
  ...
  async remoteSignIn(
  ...
  ): Promise<{ user: IUser; token: string; refreshToken: string; localisation: object }> {
    ...
    const { user, token, refreshToken, localisation, permissions } = await this.centralServer.login(
      params.email,
      params.password,
    );
