...
jest.mock('./utils/runSyncSaveTransaction', () => ({
  runSyncSaveTransaction: jest.fn(async (work: any) => {
    const entityManager = {
...
describe('MobileSyncManager pull: initial vs incremental', () => {
  ...
  it('initial sync saves from memory', async () => {
    ...
    // The save work resolves to the touched tables, for the commit-failure diagnostic
    await expect(runSyncSaveTransaction.mock.results[0].value).resolves.toEqual(
      new Set(['patients']),
    );
  });
