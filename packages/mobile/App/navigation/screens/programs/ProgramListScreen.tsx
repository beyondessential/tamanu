...
import { MenuOptionButton, MenuOptionButtonProps } from '/components/MenuOptionButton';
...
const Screen = ({ selectedPatient }: ProgramListScreenProps): ReactElement => {
  ...
  useEffect(() => {
    if (programs.length > 0) {
      setdata(programs.map(program => ({
        title: program.name,
        onPress: (): void => navigation.navigate(Routes.HomeStack.ProgramStack.ProgramTabs.name, {
          program,
        }),
        fontWeight: 500,
...
export const ProgramListScreen = compose(
  withPatient,
)(Screen);
