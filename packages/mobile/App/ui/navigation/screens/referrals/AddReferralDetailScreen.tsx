...
const DumbAddRefferalDetailScreen = ({ navigation, selectedPatient }): ReactElement => {
  ...
  useEffect(() => {
    (async (): Promise<void> => {
      ...
      if (forms.length === 0) {
        ...
        const cvdForm = await models.ReferralForm.createAndSaveOne({ title: 'CVD Primary Screening Referral' });
  ...
  const renderForm = useCallback(() => <CustomReferralForm navigation={navigation} selectedForm={selectedForm} />, [ selectedForm ]);
